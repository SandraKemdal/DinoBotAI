...
class LargeCactus:
    ...
    def draw(self,DISPLAY):
        DISPLAY.blit(self.img[self.type],self.rect)
...
def game_loop(genomes, config):
    ...
    while True:
        ...
        if ground_x <= -GROUND.get_width():
            ground_x = 0
        DISPLAY.blit(GROUND, (ground_x,ground_y))
        DISPLAY.blit(GROUND, (ground_x+GROUND.get_rect().width,ground_y))
